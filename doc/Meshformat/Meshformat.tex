...
\end{tabular}\vspace{2ex}
\begin{tabularx}{1.0\textwidth}{|lX|}\hline
\emph{SideType}:        & Side type encoding, the number of corner nodes is the last digit (triangle/quadrangle), more details see \rf{sec:CGNS}. \\
%
...
\begin{itemize}
\item \textbf{ElemInfo}: The range of sides defined by offsetIndSide and lastIndSide now includes the small mortar master sides.
\item \textbf{SideInfo}: The field nbElemID of the big mortar master defines no connection to the neighbor element, but contains the type of the mortar interface from Figure~\ref{fig:mortartypes} \emph{with negative sign}, to mark that they belong to a mortar interface. The type of the interface defines the number of the small mortar master sides.
\item \textbf{SideInfo}: Only the small mortar masters have a valid nbElemID, defining the connection to the adjacent small elements.
\item \textbf{SideInfo}: The list of sides belonging to an element includes the small mortar master sides sorted as exemplified in table \ref{tab:mortarsorting}.
